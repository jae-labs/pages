...
describe('Hero video sources', () => {
  test('uses an HLS-only source and play-triggered loading', () => {
    ...
    expect(heroComponent).toContain('controls-delay="86400000"');
    ...
    expect(heroComponent).toContain('id="hero-video-trigger"');
    expect(heroComponent).toContain('data-hero-video-trigger');
    expect(heroComponent).toContain('aria-label="Play promotional video for lui.z"');
  ...
  test('loads the focused hero client module instead of keeping inline player logic', () => {
    ...
    expect(heroComponent).not.toContain("import 'vidstack/player';");
    expect(heroComponent).not.toContain("import 'vidstack/player/layouts';");
    expect(heroComponent).not.toContain("import 'vidstack/player/ui';");
