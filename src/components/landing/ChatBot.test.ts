...
describe('ChatBot component contract', () => {
  ...
  test('includes the disabled Deep thinking button and its custom tooltip', () => {
    expect(chatBotComponent).toContain('aria-label="Deep thinking (Disabled)"');
    expect(chatBotComponent).toContain('disabled');
    expect(chatBotComponent).toContain('Enterprise feature. Contact us for unparallel reasoning capacity.');
  });
});
