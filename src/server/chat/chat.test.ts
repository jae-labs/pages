...
describe('chat API handler', () => {
  test('rejects invalid request bodies before calling upstream API', async () => {
    const fetchUpstream = vi.fn();
    const handler = createChatHandler({ fetchUpstream });
  ...
  test('sends constrained public-facts prompt to upstream API', async () => {
    const fetchUpstream = vi.fn().mockResolvedValue(
      Response.json({
        choices: [{ message: { content: 'I am a limited artificial clone of lui.z.' } }]
      })
    );
    const handler = createChatHandler({ fetchUpstream, now: () => 1_000 });
    ...
    const [, requestInit] = fetchUpstream.mock.calls[0] as [string, RequestInit];
  ...
  test('returns 429 when the KV limit is exhausted', async () => {
    const fetchUpstream = vi.fn();
    const handler = createChatHandler({ fetchUpstream, now: () => 1_700_000_000_000 });
  ...
  test('maps upstream API failures to a safe gateway error', async () => {
    const fetchUpstream = vi.fn().mockResolvedValue(Response.json({ error: 'bad upstream' }, { status: 500 }));
    const handler = createChatHandler({ fetchUpstream, now: () => 1_000 });
...
describe('Turnstile captcha verification', () => {
  ...
  test('createChatHandler rejects request if Turnstile token is invalid and secret key is set', async () => {
    ...
    const handler = createChatHandler({ fetchUpstream, fetchTurnstile });
  ...
  test('createChatHandler accepts request and calls NIM if Turnstile token is valid and secret key is set', async () => {
    ...
    const handler = createChatHandler({ fetchUpstream, fetchTurnstile, now: () => 1_000 });
