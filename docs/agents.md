...
flowchart TD
  ...
  chatLogic --> upstream["Upstream LLM Provider openai/gpt-oss-20b"]
  ...
  mediaWorker --> pages
  doppler["Doppler pages:prd/pages:rev"] --> githubOrg["GitHub organization secrets"]
  githubOrg --> release
  githubOrg --> llmSecret["PAGES_NVN_LLM_TOKEN"]
  llmSecret --> release
...
| `src/server/chat/` | Server-only chat facts, prompt, upstream completions client, request validation, and rate limiting |
...
| `../iac/cloudflare/` | Cloudflare Pages project, custom domain, DNS, and media Worker |
| `../iac/oci/` | OCI Object Storage media bucket |
| `../iac/doppler/` | Doppler project/env syncs into GitHub Actions environments |
| `../iac/github/` | GitHub repository settings, environment secrets, and collaborations |
...
- Upstream OpenAI-compatible model `openai/gpt-oss-20b` for chatbot inference.
...
- Before changing deployment, preview, DNS, media URL, or CI secret behavior, inspect the related `../iac/*` root.
...
`pnpm check` is the required final gate. It runs lint, typecheck, tests, and build.
...
- `SITE_BASE=/`
- `PAGES_NVN_LLM_TOKEN` (upstream completions API token) synced from Doppler to GitHub organization secrets, then written to Cloudflare Pages runtime secrets by CI
- `RATE_LIMIT_KV` Cloudflare KV binding for the chat endpoint
- Cloudflare Pages custom domain `justanother.engineer`
- Doppler project `pages`, configs `prd` and `rev`, synced into GitHub organization secrets
- `PAGES_PUBLIC_POSTHOG_KEY` and `PAGES_PUBLIC_POSTHOG_HOST` (public client-side variables synced via Doppler/GitHub organization secrets)
- `PAGES_PUBLIC_SENTRY_DSN` (public client-side DSN synced via Doppler/GitHub organization secrets)
- `PUBLIC_GA_TRACKING_ID` (optional public client-side Google Analytics tracking ID synced via Doppler/GitHub organization secrets)
- `PAGES_SENTRY_AUTH_TOKEN` (private Sentry authentication token synced via Doppler/GitHub organization secrets to upload build-time source maps)
- `PAGES_PUBLIC_TURNSTILE_SITE_KEY` (public client-side Turnstile site key synced via Doppler/GitHub organization secrets, defaults to Cloudflare's universal dummy sitekey in local dev)
- `PAGES_TURNSTILE_SECRET_KEY` (private server-side Turnstile secret key synced via Doppler to GitHub organization secrets, then written to Cloudflare Pages runtime secrets by CI)

Large media is served from OCI Object Storage through the Cloudflare Worker custom domain `media.justanother.engineer`; the hero promo video uses the adaptive HLS master playlist `https://media.justanother.engineer/master.m3u8`, shows a static poster preview before playback, and loads on demand when the visitor presses play. Public overrides use `PUBLIC_VIDEO_HLS_URL`.
